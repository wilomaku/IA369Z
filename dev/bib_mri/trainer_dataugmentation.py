...
class DeepNetTrainer(object):
    ...
    def fit(self, n_epochs, Xin, Yin, valid_data=None, valid_split=None, data_transf=None, batch_size=10, shuffle=True):
        if valid_data is not None:
            train_loader = DataLoader(create_dataloaders(Xin, Yin, transform=data_transf), batch_size=batch_size, shuffle=shuffle)
            valid_loader = DataLoader(create_dataloaders(*valid_data, transform=data_transf), batch_size=batch_size, shuffle=shuffle)
...
class create_dataloaders(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        if self.transform is not None:
            xi = self.transform(self.x[idx])#.repeat(3, 1, 1)#.repeat(1, 3, 1).permute(1, 0, 2)
        else:
            xi = self.x[idx]
        if self.y is None:
            yi = None
...
class ModelCheckpoint(Callback):
	...
	try:
	    os.makedirs(os.path.dirname(model_basename))
	except:
	    pass
